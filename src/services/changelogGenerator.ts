...
export class AdvancedChangelogGenerator {
  ...
  private async generateWithLLM(commits: string, version: string, context: ChangelogContext): Promise<string> {
    const prompt = `Generate a professional changelog for version ${version} based on these commits:

${commits}

Previous versions context:
${context.previousVersions.map(v => `Version ${v.version}: ${v.content.substring(0, 200)}...`).join('\n')}

Recent trends: ${context.recentTrends.focusAreas.join(', ')}
...
- Don't fabricate details not in the commits

Format as markdown starting with ## Version ${version}`;

    try {
      const response = await this.openai.chat.completions.create({
        model: 'gpt-4',
        messages: [
          { role: 'system', content: 'You are an expert technical writer creating user-friendly changelogs.' },
      ...
      return response.choices[0]?.message?.content || this.generateSimpleChangelog(commits, version);
    } catch (error) {
      throw error;
    }
  }
